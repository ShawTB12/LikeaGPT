...
import { PanelLeft, Plus, Settings, MessageSquare, HelpCircle, History } from "lucide-react"
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn(
        "flex h-14 items-center justify-between gap-2 border-b border-border p-4",
        className
      )}
      {...props}
    />
...
const SidebarBody = React.forwardRef<
  HTMLDivElement,
  React.ComponentProps<"div"> & { asChild?: boolean; fixedHeight?: boolean }
>(({ className, asChild = false, fixedHeight, ...props }, ref) => {
  const Comp = asChild ? Slot : "div"
  return (
    <Comp
      ref={ref}
      className={cn(
        ...
        fixedHeight && "h-[calc(100%-theme(spacing.14))] ",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <Separator
      ref={ref}
      className={cn("my-2 border-border/50", className)}
      {...props}
    />
  )
})
